...
@router.get("/", response_model=List[schemas.ShowAirport])
...
async def choose_flight(
    ...
    return await transportation.reserve_flight_for_passengers(
        request.user_id, request.passenger_ids, request.flight_price, request.flight_id,db
    )

@router.post("/add-passengers", response_model=schemas.Passengers)
async def new_passenger(
    user_id: int, name: str, national_id: str, age: int, gender: str, current_user: schemas.User = Depends(oauth2.get_current_user), db: AsyncSession = Depends(database.get_db)
):
    return await transportation.create_passenger(user_id, name, national_id, age, gender, db)
